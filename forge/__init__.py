...
`forge/agent/` holds the shared pieces the valley's labs are built on. Week
four's agent lives in `archive/` and imports what it needs from here, so nothing
in this lab is a toy copy.
...
# ADK 2.8 reaches Memory Bank through `vertexai.Client`, and that class now warns
# that it is deprecated. It is ADK's call, not ours, and there is nothing a
# learner can do about it — so it does not belong on their screen.
warnings.filterwarnings("ignore", message=r".*vertexai\.Client class is deprecated.*")
